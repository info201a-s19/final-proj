...
# rename columns
colnames(bac)[colnames(bac)=="?..State"] <- "State"
...
From the chart, one can see that most states' fatal crash crashes are due to drivers with a BAC greater than or equal to 0.1. The top three states by number of fatal crashes are Texas, California, and Florida. But from the table, the state size (population) correlates to the number of accidents. The most important takeaway is knowing that roughly `r round(mean(bac_table$"Percent of Drivers w/ BAC over 0.1"), digits = 2)`% of fatal car crashes in all states is due to drunk driving, meaning that these crashes are all preventable if drivers act responsibly. 
